...
export const usersApi = {
  ...
  checkNickname: async (nickname: string): Promise<{ available: boolean }> => {
    return api.get<{ available: boolean }>(`/api/users/check-nickname?nickname=${encodeURIComponent(nickname)}`);
  },
  ...
  getFollowers: async (userId: number, page = 1, limit = 20): Promise<PaginatedResponse<User>> => {
    return api.get<PaginatedResponse<User>>(`/api/users/${userId}/followers?page=${page}&limit=${limit}`);
  },

  // 사용자가 팔로우하는 목록
  getFollowing: async (userId: number, page = 1, limit = 20): Promise<PaginatedResponse<User>> => {
    return api.get<PaginatedResponse<User>>(`/api/users/${userId}/following?page=${page}&limit=${limit}`);
  },
};
